...
fn map_is_readonly(map: *const libbpf_sys::bpf_map) -> bool {
    ...
    // Yes, it's actually supposed to be *_PROG. Despite it's name.
...
fn gen_skel_datasec_getters(
    ...
    for (idx, map) in MapIter::new(object).enumerate() {
        ...
        let struct_name = format!("{}{}", obj_name, capitalize_first_letter(&name));
...
fn gen_skel_contents(debug: bool, obj: &UnprocessedObj) -> Result<String> {
    ...
    gen_skel_datasec_getters(&mut skel, object, &obj_name, false)?;
    writeln!(skel, "}}")?;
    ...
    gen_skel_datasec_getters(&mut skel, object, &obj_name, true)?;
