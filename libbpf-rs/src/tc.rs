...
/// As such a BPF program using a TC Hook (SEC("classifier")) must be operated
...
impl TcHook {
    ...
    /// if using a builder pattern to create the TcHook makes the hook 'concrete'
    ///
    ...
    pub fn create(&mut self) -> Result<Self> {
        ...
        if err != 0 && err != -(nix::errno::Errno::EEXIST as i32) {
    ...
    ///
    /// EGRESS, INGRESS, EGRESS|INGRESS, or CUSTOM
